...
public class MessageItem implements Parcelable {
	public static class ChatField {
		...
		public static final String CHAT_ISCOM="iscom";
	 ...
	 * 是否发来的消息
	 */
	@Column(name=ChatField.CHAT_ISCOM)
	...
	public void writeToParcel(Parcel dest, int flags) {
		...
		dest.writeInt(isComMsg);
		dest.writeLong(time);
		dest.writeLong(seq);
	}

	public boolean isComMsg() {
		return isComMsg==1?true:false;
	}

	public void setIsComMsg(int isComMsg) {
		this.isComMsg = isComMsg;
	}

	public static final Creator<MessageItem> CREATOR = new Creator<MessageItem>() {
		public MessageItem createFromParcel(final Parcel in) {
			return new MessageItem(in);
		}

		public MessageItem[] newArray(final int size) {
			return new MessageItem[size];
		}
	};

	// 根据seq排序
	public static class ComparatorPY implements Comparator<MessageItem> {
